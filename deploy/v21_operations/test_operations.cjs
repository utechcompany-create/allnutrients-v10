...
function chatContext(){
 ...
 const fetch=async(path,options={})=>{calls.push({path,options});let data;
   if(path==='/api/auth/me')data={authenticated,id:'admin',role:'admin'};
 ...
 return {chat:window.ChatUI.mount(root,{admin:true}),root,events,document,streams,calls,history,m,intervals,setAuthenticated(v){authenticated=v},defer(p){hold=p}};
}
(async()=>{
 ...
 console.log('PASS: live chat UI, safe rendering, simultaneous delivery, private room switching, stale responses, expired sessions and navigation');
