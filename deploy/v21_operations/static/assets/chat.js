(function(){
  ...
  function mount(root,{admin=false}={}){
    root.innerHTML=`<div class="chat-layout group"><aside class="chat-rooms"><h3>1:1 대화방</h3>${admin?'<form class="chat-start"><label>고객 선택<select aria-label="상담 고객"><option value="">고객 선택</option></select></label><button class="btn ghost">새 상담 시작</button></form>':''}<div class="chat-room-list"></div></aside><section class="chat-panel"><div class="chat-heading"><h2>대화방</h2><p class="chat-notice sales-note"></p><div class="chat-status" role="status"></div></div><button class="btn ghost small chat-older" hidden>이전 대화 보기</button><div class="chat-log" role="log" aria-label="대화 내용" aria-live="polite"></div><form class="chat-compose"><label>메시지<textarea maxlength="2000" rows="2" required placeholder="메시지를 입력하세요. (최대 2,000자)" disabled></textarea></label><button class="btn primary" disabled>전송</button></form><p class="chat-error" role="alert"></p></section></div>`;
    ...
    function clear(){room=null;messages.clear();cursor=0;log.innerHTML='';list.innerHTML='';input.value='';input.disabled=true;send.disabled=true;older.hidden=true;pending=null;sending=false;}
    ...
    function render(batch,prepend=false,advance=true){const near=log.scrollHeight-log.scrollTop-log.clientHeight<100,previous=log.scrollHeight;for(const m of batch)messages.set(m.id,m);const ordered=[...messages.values()].sort((a,b)=>a.id-b.id);if(advance&&batch.length)cursor=Math.max(cursor,...batch.map(m=>m.id));log.innerHTML=ordered.length?ordered.map(messageHTML).join(''):'<p class="muted">첫 메시지를 보내 대화를 시작하세요.</p>';if(prepend)log.scrollTop+=log.scrollHeight-previous;else if(near||batch.some(m=>m.isMine))log.scrollTop=log.scrollHeight;}
    ...
    async function selectRoom(next){
      stop();const g=generation;clear();room=next;error.textContent='';status.textContent='대화를 불러오는 중입니다.';$('.chat-heading h2').textContent=next.name;
    ...
    async function roomList(g){const rows=await request('/api/chat/rooms');if(g!==generation)return;list.innerHTML=rows.filter(x=>x.kind==='DIRECT').map(x=>`<button class="chat-room ${room?.id===x.id?'active':''}" data-room="${esc(x.id)}"><b>${esc(x.name)}</b><small>${esc(x.lastMessage||'새 대화방')}</small></button>`).join('')||'<p class="muted">접수된 1:1 상담이 없습니다.</p>';return rows;}
    let ticks=0;
    ...
    async function open(nextKind='GROUP'){
      active=true;kind=nextKind;stop();clear();const g=generation;error.textContent='';layout.classList.toggle('group',kind==='GROUP');$('.chat-heading h2').textContent=kind==='GROUP'?'전체 대화방':'1:1 대화방';$('.chat-notice').textContent=kind==='GROUP'?'모든 회원과 관리자가 볼 수 있습니다. 개인정보는 1:1 상담으로 보내 주세요.':'해당 고객과 관리자만 대화 내용을 볼 수 있습니다.';status.textContent='대화방을 불러오는 중입니다.';
    ...
    list.onclick=async e=>{const b=e.target.closest('[data-room]');if(!b)return;const g=generation;try{const rows=await request('/api/chat/rooms');if(g===generation){const next=rows.find(x=>x.id===b.dataset.room);if(next)await selectRoom(next)}}catch(err){if(g===generation)fail(err)}};
