(function () {
  ...
  function renderHistory() {
    ...
    $('requestHistoryList').innerHTML=requests.length?requests.map(r=>`<article class="req"><div class="row"><b>${esc(r.requestNo)} · ${r.requestType==='RETURN'?'반품':'교환'}</b><span class="badge">${esc(ReturnProgress.statusLabel(r,statusKo[r.status]||r.status))}</span></div><p class="muted">신청일 ${esc(dateText(r.createdAt))}${r.completedAt&&(r.requestType!=='RETURN'||ReturnProgress.completed(r)||ReturnProgress.stopped(r))?' · 처리 완료일 '+esc(dateText(r.completedAt)):''}</p><p>${(r.items||[]).map(i=>esc(i.name)+' × '+Number(i.qty)).join('<br>')}</p><p>사유: ${esc(r.reasonName||r.reasonCode)}<br>${esc(r.reasonDetail||'')}</p><p>${r.requestType==='RETURN'?'반품배송비 차감':'교환배송비 고객 부담'}: ${money(r.returnShippingFee)}</p>${r.requestType==='RETURN'?`<p>${['REJECTED','CANCELED'].includes(r.status)?'환불 미진행':ReturnProgress.completed(r)?'환불 완료 금액: '+money(r.refundAmount):'환불 예정 금액: '+money(r.refundAmount)}</p>`:''}${r.requestType==='RETURN'?`<p>환불 상태: ${esc(ReturnProgress.refundLabel(r))}</p><p>입고 완료일: ${ReturnProgress.received(r)?esc(dateText(r.receivedAt)):'입고 확인 전'}</p>`:''}${r.pickup?.trackingNumber?`<p class="muted">회수: ${esc(r.pickup.carrier)} ${esc(r.pickup.trackingNumber)}</p>`:''}${r.exchangeShipment?.trackingNumber?`<p class="muted">교환 재배송: ${esc(r.exchangeShipment.carrier)} ${esc(r.exchangeShipment.trackingNumber)}</p>`:''}${r.adminNote?`<p class="notice">판매자 안내: ${esc(r.adminNote)}</p>`:''}<div class="photos">${(r.attachments||[]).filter(x=>typeof x==='string'&&x.startsWith('/uploads/')).map(x=>`<a href="${esc(x)}" target="_blank" rel="noopener"><img src="${esc(x)}" alt="신청 첨부 사진"></a>`).join('')}</div></article>`).join(''):'<p class="muted">신청 내역이 없습니다.</p>';
  }
