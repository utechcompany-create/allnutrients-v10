(function () {
  ...
  const completed = ReturnProgress.completed;
  ...
  function card(r) {
    ...
    return `<article class="return-card" aria-label="${esc(r.requestNo)} 반품 내역">
      <div class="return-heading"><div><h3>${esc(r.requestNo)}</h3><p class="muted">주문번호 ${esc(r.orderNo)}</p></div><span class="badge ${done?'return-done':stopped?'warn':''}">${esc(ReturnProgress.statusLabel(r,statusNames[r.status] || r.status))}</span></div>
      ...
      <dl class="return-details">
        ...
        ${detail('입고 완료일', ReturnProgress.received(r) ? dateText(r.receivedAt) : '입고 확인 전')}
        ${detail('환불 상태', ReturnProgress.refundLabel(r))}
      ...
      ${ReturnProgress.needsCheck(r)?'<p class="notice">입고·환불 기록 확인이 필요합니다. 판매자에게 문의해 주세요.</p>':''}
  ...
  async function load() {
    ...
    try {
      const result = await api('/api/returns/my', {cache:'no-store'});
